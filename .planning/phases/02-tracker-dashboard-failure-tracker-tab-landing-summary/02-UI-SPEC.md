...
|-------|-------|------------------|
| xs | 4px | Table cell horizontal padding, TH vertical padding, gap between dimension counters |
| sm | 8px | Table cell vertical padding, row gap inside tracker entries, gap between table rows, margin below table header, tab pane top padding |
| md | 12px | Section break between "limpiar todo" header and table |
| lg | 16px | Empty state vertical padding |

**Exceptions (inherited, not net-new):**
- The landing pill reuses `.ev-pill` geometry (`padding:1px 6px; font-size:9px`) — these values are inherited from the existing class and are NOT new declarations by this phase.
- Unblock button min-height is 24px (smaller than the 38px `.btn` used elsewhere) because it lives inside a table row. Uses a new compact class `.tr-btn` — do not alter `.btn`. 24 is a multiple of 4.

**Net-new values introduced by this phase: 4, 8, 12, 16, 24 — all multiples of 4.**
...
**Landing pill (`⊘ N fechas bloqueadas`):** reuses existing `.ev-pill` class as-is. Its typography (9px, whatever weight `.ev-pill` already declares) is inherited from the existing class — **not a new weight declaration by this phase.** The phase introduces zero typography changes to `.ev-pill`.
...
- `⊘ N fechas bloqueadas` uses `background:rgba(248,113,113,.1); color:var(--red); border:1px solid rgba(248,113,113,.2)` — this is exactly `.ev-err` reused. **Decision: reuse `.ev-err`** — the semantics (red = bad) are consistent enough, and this keeps the CSS footprint at zero for the landing change.
...
- **Class:** `<span class="ev-pill ev-err">⊘ N fechas bloqueadas</span>` (reuses existing class — no typography or spacing changes).
...
.tr-btn{font-family:inherit;font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.3px;
  padding:4px 8px;border-radius:4px;cursor:pointer;-webkit-tap-highlight-color:transparent;
...
- Dashboard max-width is 540px (mobile-first). Tracker table must fit: 8 columns at `font-size:11px` in monospace is ~48 characters — tight but viable. Wrap the table in `<div class="tr-tbl-wrap">` (`overflow-x:auto`) — NO column hiding, operators need all dimensions visible.
