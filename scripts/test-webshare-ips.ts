...
const TARGETS = [
  { url: 'https://ais.usvisa-info.com/es-pe/niv/users/sign_in', label: 'sign_in (es-pe)' },
  { url: 'https://ais.usvisa-info.com/es-co/niv/users/sign_in', label: 'sign_in (es-co)' },
];
...
async function testIp(proxy: { label: string; ip: string; port: number }, targetUrl: string, targetLabel: string): Promise<TestResult> {
  ...
  try {
    ...
    const isReal = hasSignIn || hasJson;
...
for (let round = 1; round <= rounds; round++) {
  ...
  for (const proxy of ips) {
    ...
    for (const target of TARGETS) {
      const result = await testIp(proxy, target.url, target.label);
