...
for (const win of [20,10,5,3,1]) {
  ...
  meta AS (
    SELECT id, visa_category, jsonb_array_length(applicant_ids) as napp FROM bots
  )
  SELECT ma.visa_category as cat_a, mb.visa_category as cat_b,
         count(*) as n_pairs,
         round(avg(
  ...
  JOIN polls b ON a.bot_id < b.bot_id
    AND abs(extract(epoch from a.created_at - b.created_at)) <= ${sql.raw(String(win))}
  JOIN meta ma ON ma.id = a.bot_id
  JOIN meta mb ON mb.id = b.bot_id
  WHERE (ma.visa_category='B1/B2' AND mb.visa_category='B1/B2' AND ma.napp=1 AND mb.napp=1)
  ...
  GROUP BY ma.visa_category, mb.visa_category
  ORDER BY cat_a, cat_b
  `);
  console.log('window', win, 's');
